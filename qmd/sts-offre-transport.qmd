...
- Offre: `r total_lignes` lignes, `r total_arrets` arrets, `r total_trajets` trajets.
- Service: debut a `r first_bus_time`, fin a `r last_bus_time` (amplitude `r round(amplitude_hours, 1)` h).
- Concentration: top 10 lignes = `r round(sum(top_10_lines$Part_Trajets) * 100, 1)` % des trajets.
- Pointe: heure la plus chargee a `r peak_hour$heure` h (matin/soir dominants).
...
### Analyse des heures de pointe (matin/soir)

Lecture rapide des pics de service sur les plages 06h-09h et 16h-19h.


- Heure la plus chargee: `r peak_hour$heure` h
- Part des departs 06h-09h: `r percent(morning_share)`
- Part des departs 16h-19h: `r percent(evening_share)`
...
### Distribution de l'amplitude horaire par ligne
...
- L'amplitude horaire mediane par ligne est de `r round(amplitude_stats$median_amp, 1)`
  heures, avec un maximum de `r round(amplitude_stats$max_amp, 1)` heures.

## Etape 5: Complement d'analyse

### Lignes a forte couverture d'arrets
...
- Donnees d'offre uniquement (pas de demande voyageurs).
- Periode temporelle limitee aux fichiers GTFS disponibles.
- Qualite des heures dependante du renseignement initial.

## Conclusion

Constats:
- Les trajets sont concentres sur un petit nombre de lignes.
- La distribution des frequences est heterogene selon les lignes.
- Les departs se concentrent sur des plages horaires de pointe (matin/soir).

Recommandations:
- Renforcer l'offre sur les lignes les plus sollicitees.
- Ajuster la frequence pendant les heures de pointe matin/soir.
- Ameliorer la qualite des donnees horaires pour un suivi plus fin.
